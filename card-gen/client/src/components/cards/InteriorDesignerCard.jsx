...
const InteriorDesignerCard = ({ cardData, hiddenFields = [], cardId }) => {
    ...
    <div className="bg-[#1F1B24] border-2 border-pink-200 rounded-xl p-6 shadow-lg w-[24rem] mx-auto">
      ...
      {!isFieldHidden('aboutUs') && (
        <div className="mb-4">
          <h4 className="text-xl font-bold text-white mb-2 flex items-center">
            <img height={16} width={16} src="/interior-designer/about-us.svg" alt="About Us" className="mr-1 h-5 w-5" />
            About US
          </h4>
          ...
          {Array.isArray(cardData.certifications) && cardData.certifications.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {cardData.certifications.map((cert, idx) => (
                <div key={idx} className="text-xs text-[#E74C3C] bg-white rounded-full py-[2px] px-2 border border-[#E74C3C] hover:bg-pink-400 hover:border-pink-800 hover:text-white transition-all duration-300">
                  {cert}
                </div>
              ))}
            </div>
      ...
      {!isFieldHidden('workImages') && Array.isArray(cardData.workImages) && cardData.workImages.length > 0 && (
        <div className="mb-6 pt-4 border-t border-pink-200">
          ...
          <img height={16} width={16} src="/interior-designer/our-work.svg" alt="About Us" className="mr-1 h-5 w-5" />
            Our Work
          </h4>
          ...
          {Array.isArray(cardData.clientLogos) && cardData.clientLogos.length > 0 ? (
            <div className="flex gap-2 items-center overflow-x-auto scrollbar-hide">
              {cardData.clientLogos.map((url, idx) => (
                <div key={`${url}-${idx}`} className="h-16 min-w-[90px] flex items-center justify-center bg-white rounded border border-pink-200">
              ...
              {cardData.teamMembers.map((member, idx) => (
                <div key={idx} className="rounded-lg py-0.5 space-y-2 flex flex-col items-center">
                  ...
                  <div className='space-y-0.5'>
                    <p className="text-xs font-medium text-white text-center">{member.name || 'Team Member'}</p>
                    <p className="text-[10px] text-gray-300 text-center">{member.post || 'Position'}</p>
                  </div>
                </div>
              ))}
            </div>
...
export default InteriorDesignerCard;
