...
const TravelAgentCard = ({ cardData, hiddenFields = [], cardId }) => {
  ...
  return (
    <div className="bg-[#1F1B24] border-2 border-pink-200 rounded-xl p-6 shadow-lg w-[24rem] mx-auto">
      ...
      {!isFieldHidden('aboutUs') && (
        <div className="mb-4">
          <h4 className="text-xl font-bold text-white mb-2 flex items-center">
            <img height={16} width={16} src="/travel-agent/about-us.svg" alt="About Us" className="mr-1 h-5 w-5" />
            About US
          </h4>
      ...
      {/* Certifications */}
      {!isFieldHidden('certifications') && cardData.certificationsText && (
        <div className="mb-6">
          <div className="flex items-center mb-4">
            <h4 className="text-xl font-bold text-white flex items-center pr-1">
              <img height={16} width={16} src="/travel-agent/certified.svg" alt="Certified" className="mr-1 h-6 w-6" />
              Certified
            </h4>
            {!isFieldHidden('certificationsText') && cardData.certificationsText && (
              <div className="ml-2">
                <p className="text-sm text-white py-1 px-2 border-l-2 border-gray-500">
          ...
          {Array.isArray(cardData.certifications) && cardData.certifications.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {cardData.certifications.map((cert, idx) => (
                <div key={idx} className="text-xs text-[#E74C3C] bg-white rounded-full pb-[2px] px-2 border border-[#fc5a48] hover:bg-[#fc5a48] hover:text-white transition-all duration-300">
                  {cert}
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              {cardData.features.map((feature, idx) => (
                <div key={idx} className="flex flex-col items-center gap-2 transition-all duration-300 group">
                  {feature.image && (
                    <div className="w-10 h-10 rounded-full overflow-hidden flex-shrink-0 transition-all duration-300">
                  ...
                  <div className="min-w-0">
                    ...
                    {feature.description && (
                      <p className="text-xs text-white whitespace-pre-wrap break-words text-center leading-[1.15] mt-1 group-hover:text-gray-200 transition-colors duration-300">{feature.description}</p>
                    )}
                  </div>
                </div>
              ))}
      ...
      {!isFieldHidden('destinations') && Array.isArray(cardData.destinations) && cardData.destinations.length > 0 && (
        <div className="mb-6 pt-4 border-t border-green-200">
          <div className="flex justify-between items-center mb-4">
          <h4 className="text-xl font-semibold text-white flex items-center">
            <img height={16} width={16} src="/travel-agent/popular-destinations.svg" alt="Popular Destinations" className="mr-1 h-5 w-6" />
            Popular Destinations
          </h4>
          ...
          <div className="mb-4">
            <div className="flex space-x-2 overflow-x-auto pb-2">
              {cardData.destinations.map((destination, idx) => (
                <button
                  key={idx}
                  ...
                  className={`flex-shrink-0 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 ${
                    selectedDestinationIndex === idx
                      ? 'bg-white text-green-600 hover:shadow-md'
          ...
          {cardData.destinations[selectedDestinationIndex] && (
            <div className="space-y-3">

              
              {cardData.destinations[selectedDestinationIndex].packages && 
               Array.isArray(cardData.destinations[selectedDestinationIndex].packages) && 
               cardData.destinations[selectedDestinationIndex].packages.length > 0 ? (
                <div className="space-y-6">
                  {cardData.destinations[selectedDestinationIndex].packages.map((pkg, pkgIdx) => (
                    <div key={pkgIdx} className="hover:shadow-lg transition-all duration-300 group">
                      {pkg.packageImage && (
                        <div className="w-full h-36 mb-2 rounded-xl overflow-hidden border border-transparent group-hover:border-green-300 transition-all duration-300">
                          <img src={pkg.packageImage} alt={pkg.packageName} className="w-full h-full object-cover transition-transform duration-300" />
                        </div>
                      )}
                      <div className="flex justify-between items-start">
                        <div className="w-full">
                          <div className="flex justify-between items-start">
                            <p className="text-lg font-medium text-white group-hover:text-green-300 transition-colors duration-300">{pkg.packageName}</p>
                            {pkg.price && (
                              <span className="text-sm text-[#FFDE00] font-semibold flex flex-col items-end group-hover:text-yellow-300 transition-colors duration-300">
                                Rs. {pkg.price}
                                {/* <span className="text-[11px] text-[#FFDE00] leading-[1]">Package</span> */}
                              </span>
                            )}
                          </div>
                          {pkg.description && (
                            <p className="text-sm text-gray-100 group-hover:text-gray-200 transition-colors duration-300">{pkg.description}</p>
                          )}
                        </div>
                      </div>
                    </div>
                ...
                <div className="text-center py-8">
                  <p className="text-gray-300 text-sm">No packages available for this destination</p>
                </div>
              )}
            </div>
          )}
      ...
      {!isFieldHidden('services') && (
        <div className="mb-6 pt-4 border-t border-pink-200">
          <div className='flex justify-between items-center mb-3'>
            ...
            {/* <img height={16} width={16} src="/interior-designer/design-services.svg" alt="Services" className="mr-1 h-5 w-5" /> */}
             Our Services
            </h4>
            <a href={getFieldValue('website', 'Not provided')} className="text-xs text-white hover:text-pink-300 duration-300 hover:underline font-medium transition-colors">View All</a>     {/* Client's Website link will be added*/}
          </div>
          {Array.isArray(cardData.services) && cardData.services.length > 0 ? (
            <div className="space-y-2">
              {cardData.services.map((service, idx) => (
                <div key={idx} className="rounded-xl flex items-center gap-4 justify-between hover:bg-[#ffffff37] hover:shadow-md transition-all duration-300 group">
                  <div className='flex items-center gap-4 p-2 w-full'>
                    {service.serviceImage && (
                      <div className="w-12 aspect-square rounded-full overflow-hidden transition-all duration-300">
                        <img src={service.serviceImage} alt={service.serviceName} className="w-full h-full object-cover transition-transform duration-300" />
                      </div>
                    )}
                    <div className="space-y-1 w-full">
                      <div className="flex justify-between items-center">
                        <span className="text-sm font-medium text-white group-hover:text-pink-200 transition-colors duration-300">{service.serviceName || 'Service Name'}</span>
                        {service.price && (
                        <span className="text-sm text-pink-100 font-semibold flex items-center group-hover:text-pink-200 transition-colors duration-300">
                          Rs. {service.price}
                        </span>
                    )}
                      </div>
                      {service.serviceDescription && (
                        <p className="text-xs text-gray-300 group-hover:text-gray-200 transition-colors duration-300">{service.serviceDescription}</p>
                      )}
                    </div>
                  </div>
                </div>
      ...
      <div className="mb-6 pt-4 border-t border-pink-200">
        <h4 className="text-xl font-bold text-white mb-3 flex items-center">
          <img height={16} width={16} src="/travel-agent/contact-us.svg" alt="Contact Us" className="mr-1 h-6 w-6" />
          Contact Us
        </h4>
        <div className="grid grid-cols-2 gap-2 mb-4 border-b border-pink-200 pb-4">
          ...
          {!isFieldHidden('address') && (
            <a href={cardData.mapEmbedLink} target="_blank" rel="noopener noreferrer" className="flex items-center space-x-2 text-base  border border-[#453268] rounded-lg bg-[#2E2438] px-4 py-2 hover:bg-[#3a2f47] hover:border-[#5a4178] hover:shadow-md transition-all duration-300 group">
        ...
        {!isFieldHidden('mapEmbedLink') && cardData.mapEmbedLink && (
          <div className="mt-2 flex gap-4">
            ...
            <div className="">
              ...
              <p className="text-white text-xs">{getFieldValue('address', 'Not provided')}</p>
      ...
      {!isFieldHidden('galleryImages') && (
        <div className="mb-6 pt-4 border-t border-pink-200">
          <h4 className="text-lg font-semibold text-white mb-4 flex items-center justify-between">
            ...
            <span>Gallery</span>
            <a href={getFieldValue('website', 'Not provided')} className="text-xs text-gray-100">View All</a>
          </h4>
          {Array.isArray(cardData.galleryImages) && cardData.galleryImages.length > 0 ? (
            <div className="grid grid-cols-2 gap-2">
              {cardData.galleryImages.map((url, idx) => (
                <div key={`${url}-${idx}`} className="w-full aspect-square overflow-hidden rounded-lg border border-white hover:shadow-lg hover:border-pink-400 transition-all duration-300 group bg-white">
                  <img src={url} alt={`gallery-${idx}`} className="w-full h-full object-cover transition-transform duration-300" />
                </div>
              ))}
            </div>
          ) : (
      ...
      {!isFieldHidden('testimonials') && (
        <div className="mb-6 pt-4 border-t border-pink-200">
          ...
          <img height={16} width={16} src="/interior-designer/testimonials.svg" alt="Testimonials" className="w-6 h-6" />
          <p className="text-white text-lg font-medium">Client Testimonials</p>
          </div>
            ...
            <div className="space-y-3 mt-2">
              {cardData.testimonials.map((testimonial, idx) => (
                <div key={idx} className="bg-transparent w-[85%] mx-auto">
                  ...
                  <p className="text-xs text-white italic mt-2">"{testimonial.reviewText || 'Great service!'}"</p>
      ...
      {!isFieldHidden('ctaTitle') && (
        <div className="mb-12 pt-4 text-center">
          <h4 className="text-lg font-semibold text-white mb-1">
            {getFieldValue('ctaTitle', 'Ready for Your Next Adventure?')}
          </h4>
          {!isFieldHidden('ctaSubtitle') && (
            <p className={`text-5xl font-bold mb-3 ${isFieldEmpty('ctaSubtitle') ? 'text-white' : 'text-white'}`}>
              {getFieldValue('ctaSubtitle', 'Contact Us Today!!')}
            </p>
          )}
